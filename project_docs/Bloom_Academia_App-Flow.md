...
**Multi-Agent System (Behind the Scenes):**
The teaching experience is powered by a 9-agent AI system that works seamlessly:
- **Coordinator**: Routes questions to the right specialist (invisible to student)
- **Subject Specialists**: Math, Science, English, History, Art (students interact with these)
- **Support Agents**: Assessor (checks mastery), Motivator (provides encouragement), Validator (prevents errors)

Each agent has distinct voice and teaching style. Handoffs happen smoothly when changing subjects.
...
•	Content appears as the AI teaches:
   • SVG diagrams (pizzas, shapes, number lines) - generated on-the-fly by AI
   • Mathematical equations
   • Highlighted text and markdown-formatted explanations
   • Practice problems
   • When available, web sources are cited at bottom (History/Science only)

SVGs appear instantly when audio playback starts. Simple and crisp rendering.
...
Bottom-right corner:
•	Progress indicator shows teaching phase (1-5)
...
8. Parallel processing:
   ...
   - SVG code extracted for whiteboard
   - Evidence extracted and mastery recorded (fire-and-forget)
   - Profile enrichment triggered if patterns detected (fire-and-forget)
9. Frontend receives: audioBase64 + displayText + SVG + agentName
10. Plays audio, renders SVG, shows formatted text

Step 1: Welcome & Introduction

The lesson starts. Whiteboard is blank.

Math Specialist speaks: "Hi [Name]! I'm so excited to teach you about fractions today. Have you ever shared a pizza with friends? Well, that's exactly what fractions are about - sharing things into equal parts. Let's dive in!"

While AI speaks:
•	Lesson title and explanation appear on screen (markdown formatted)
•	A pizza SVG diagram appears on whiteboard (generated by AI)
•	Voice indicator shows pulsing animation: "Teaching..."

Student can interrupt at any time by just speaking. If they say "Wait, I don't understand," the AI pauses and adjusts explanation based on detected confusion.
...
8.1 Data Persistence (Supabase Database)
**Actual Implementation:**
All data persists in Supabase PostgreSQL database:

**Tables Used:**
- `users`: Student profiles (name, age, grade, learning_style, strengths, struggles, preferences)
- `sessions`: Learning session tracking (started_at, ended_at, effectiveness_score)
- `interactions`: Every conversation turn (user_message, ai_response, timestamp)
- `lessons`: All lesson metadata (title, subject, grade_level, learning_objectives)
- `progress`: Per-lesson progress tracking (mastery_level, time_spent, completed)
- `mastery_evidence`: Learning analytics (evidence_type, strength, confidence)
- `trajectory_snapshots`: Historical performance trends

**Authentication:**
- Supabase Auth with email/password and Google OAuth
- User ID tied to all data for cross-device sync

**Memory System (3 Layers):**
1. **User Profile** (persistent): Learning style, strengths, struggles - updated in real-time
2. **Session History** (temporary): Last 5 interactions for context continuity
3. **Real-time Enrichment** (dynamic): Profile updates mid-session when patterns detected

No localStorage needed - everything in database for cross-device access.
